...
- 現在：先盤點、再 inspect、再 diff，最後才 replay 或釋出，順序更清楚。
...
| 釋出草稿 | `review`, `publish`, `edit-live` | 本地 dashboard JSON | dry-run / publish 結果 | apply 後再 list / get |
...
`provisioning/` 是部署投影。它讓 Grafana 用 provisioning 設定從檔案系統讀 dashboard，適合被放進容器映像、ConfigMap、volume mount 或 GitOps-style 部署流程。這條路徑可以部署，但不適合作為日常 review 與回放的唯一真相來源。

Grafana Git Sync 也是 source-owned lane。當 folder 是 Git Sync-managed，dashboard JSON 的擁有者是 repository 與 PR flow。`grafana-util` 可以用來 inspect、export evidence、plan 與 validate dependencies，但實際部署應更新 Git Sync repository 或 provisioning source，不要強制走 `dashboard import` 或 `workspace apply --execute-live`。
...
| `provisioning/` | **檔案配置** | 供 Grafana 用其內建配置系統從磁碟讀取 Dashboard。 |
...
- 不要把 `$datasource` 直接解讀成 mixed datasource family；很多情況只是 panel 仍用同一個 datasource variable 做選擇。
...
- **即時工作 (Live)**：針對 API-managed dashboards 直接對接 Grafana 的盤點、即時 Diff、匯入與刪除。

**黃金守則**：先用 `list` 或 `browse` 發現資產，`export` 到暫存目錄，用 `dashboard dependencies` 與 `diff` 驗證，最後在 Dry-run 符合預期後才執行 `import` 或 `delete`。

如果 discovery 或 plan output 顯示 dashboard 是 Git Sync-managed 或 file-provisioned，請在 live mutation 前停下來。下一步應該是 repository/PR 或 provisioning change，而不是直接 dashboard API replay。
...
若 folder 屬於 Git Sync 管理，dashboard JSON 應保留在同步的 repository，讓 Grafana 從那裡套用。`raw/` export 仍可作為 review 或 migration evidence，但不應變成繞過 Git-owned workflow 的捷徑。
...
| **相依性 (Dependencies)** | `grafana-util dashboard dependencies --input-dir ./dashboards/raw --input-format raw --output-format text` |
