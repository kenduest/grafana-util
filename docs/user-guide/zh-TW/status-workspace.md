...
Alerts       ok       無孤立規則
```
`status live` 走的是 live Grafana 聚合 / 讀取路徑。若同時帶入 staged sync 或 package-test 檔案，只是在這個讀取結果中補上 desired-vs-live 對照脈絡；它不會變成 workspace preview 或 apply。
...
Git Sync 與 file-provisioned dashboard inputs 應視為 source-owned。Workspace scan、test、preview 可以作為這些 tree 的 review gate；但如果 preview 指向 Git Sync-managed 或 file-provisioned target，live change 應回到 repository/PR 或 provisioning path，不要用 `workspace apply --execute-live` 直接寫入。
...
`status overview live --output-format interactive` 會用共用的 status live 讀取路徑顯示 live Grafana overview。
...
如果要看 staged 產物的人工審查畫面，用不帶 `live` 的 `status overview`；如果要拿結構化輸出做 live Grafana 聚合，改用 `status live`。
